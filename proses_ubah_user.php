...
if($_POST){
    ...
    if(empty($nama_user)){
        ...
        if(!empty($role)){
            echo "tes";
            $update=mysqli_query($conn,"update user set nama_user='".$nama_user."', username='".$username."', password='".$password."', role='".$role."' where id_user = ".$id_user." ") or die(mysqli_error($conn));
            if($update){
